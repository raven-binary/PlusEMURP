            ...
            get
            {
                ...
                switch (GetBaseItem().InteractionType)
                {
                    ...
                    case InteractionType.FISH:
                        return new InteractorFish();

                    case InteractionType.SELLFISH:
                        return new InteractorSellFish();

                    case InteractionType.SHOP:
                        return new InteractorShop();

                    case InteractionType.FARM:
                        return new InteractorFarm();

                    case InteractionType.LOCKPICK:
                        return new InteractorLockpick();

                    case InteractionType.SELLWEAPON:
                        return new InteractorSellWeapon();

                    case InteractionType.PHONESTORE:
                        return new InteractorPhoneStore();

                    case InteractionType.CAFE:
                        return new InteractorCafe();

                    case InteractionType.HOSPITAL:
                        return new InteractorHospital();
                    ...
                    case InteractionType.BIN:
                        return new InteractorBin();

                    case InteractionType.BANKMENU:
                        return new InteractorBankMenu();

                    case InteractionType.POLICE:
                        return new InteractorPolice();

                    case InteractionType.JUKEBOX:
                        return new InteractorJukebox();

                    case InteractionType.APPLEENERGY:
                        return new InteractorAppleEnergy();

                    case InteractionType.CORPTASK:
                        return new InteractorCorpTask();

                    case InteractionType.ARMOURY:
                        return new InteractorArmoury();

                    case InteractionType.ROBCASH:
                        return new InteractorRobCash();

                    case InteractionType.INFO_TERMINAL:
                        return new InteractorInfoTerminal();
                        ...
                        case InteractionType.TELEPORT:
                            {
                                ...
                                {
                                    User = GetRoom().GetRoomUserManager().GetRoomUserByHabbo(InteractingUser);
                                    if (User.GetClient().GetHabbo().JobId == 4 && User.GetClient().GetHabbo().RankId == 1 && User.GetClient().GetHabbo().MenottedUsername != null)
                                    {
                                        User.GetClient().SendWhisper("Du kannst die Polizeistation nicht verlassen, während du einem Bürger Handschellen anlegst.");
                                        User.UnlockWalking();
                                        return;
                                    }
                                        ...
                                        if (User.Coordinate == Coordinate)
                                        {
                                            ...
                                            if (ItemTeleporterFinder.IsTeleLinked(Id, GetRoom()))
                                            {
                                                ...
                                                if (true)
                                                {
                                                    ...
                                                    {
                                                        User.GetClient().SendWhisper("Du kannst dieser Raum momentan nicht betreten, da dort ein Salat organisiert wird.");
                                                        User.UnlockWalking();
                                                        return;
                                                    }
