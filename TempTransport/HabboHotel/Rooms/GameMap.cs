...
namespace Plus.HabboHotel.Rooms
{
    public class Gamemap
    {
        ...
        public bool AddItemToMap(Item Item, bool handleGameItem, bool NewItem = true)
        {
            ...
            {
                Return = (ConstructMapForItem(Item, coord) ? true : false);
            }
            return Return;
        }
        ...
        internal bool Path(int X, int Y, RoomUser user)
        {
            return IsValidStep2(user, new Vector2D(X, Y), new Vector2D(X, Y), false, false);
        }
            ...
            if (Items.Count > 0)
            {
                ...
                if (HasGroupGate)
                {
                    ...
                    if (I != null)
                    {
                        ...
                        if (Group.IsMember(User.GetClient().GetHabbo().Id) && User.GetClient().GetHabbo().Working || User.GetClient().GetHabbo().JobId == 2 && User.GetClient().GetHabbo().Working || User.GetClient().GetHabbo().JobId == 1 && User.GetClient().GetHabbo().Working)
                        {
                ...
                bool HasPrisonGate = Items.ToList().Where(x => x.BaseItem == 362).ToList().Count() > 0;
                if (HasPrisonGate)
                {
                    if ((User.IsBot) || (!User.IsBot && (User.GetClient().GetHabbo().JobId == 1 && User.GetClient().GetHabbo().Working || User.GetClient().GetHabbo().Escort)))
                        return true;
                }
            }
