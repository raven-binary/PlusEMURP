...
namespace Plus.HabboHotel.Pathfinding
{
	...
	{
		public static Vector2D[] DiagMovePoints = new Vector2D[8]
		{
			new Vector2D(-1, -1),
			new Vector2D(0, -1),
			new Vector2D(1, -1),
		...
		public static Vector2D[] NoDiagMovePoints = new Vector2D[4]
		{
			new Vector2D(0, -1),
			new Vector2D(1, 0),
			new Vector2D(0, 1),
		...
		public static PathFinderNode FindPathReversed(RoomUser User, bool Diag, Gamemap Map, Vector2D Start, Vector2D End)
		{
			...
			while (OpenList.Count > 0)
			{
				...
				{
					Vector2D Tmp = Current.Position + (Diag ? DiagMovePoints[i] : NoDiagMovePoints[i]);
					bool IsFinalMove = Tmp.X == End.X && Tmp.Y == End.Y;
					bool DiagMovement = i == 0 || i == 2 || i == 4 || i == 6;
					if (!Map.IsValidStep(User, new Vector2D(Current.Position.X, Current.Position.Y), Tmp, IsFinalMove, User.AllowOverride, DiagMovement))
					{
						continue;
					}
					PathFinderNode Node;
					...
					int Cost = Current.Cost + Diff + Node.Position.GetDistanceSquared(End);
					if (Cost < Node.Cost)
					{
						Node.Cost = Cost;
						Node.Next = Current;
		...
		{
			return Convert.ToInt32(Math.Sqrt(Math.Pow(p2.X - p1.X, 2.0) + Math.Pow(p2.Y - p1.Y, 2.0)));
		}

		public static int GetDistance(int x, int y, int toX, int toY)
		{
			return Convert.ToInt32(Math.Sqrt(Math.Pow(toX - x, 2.0) + Math.Pow(toY - y, 2.0)));
		}
	}
}
