...
{
    class SellHospitalCommand : IChatCommand
    {
        ...
        public void Execute(GameClient Session, Room Room, string[] Params)
        {
            if (Params.Length == 1)
            {
                Session.SendWhisper("Invalid syntax :sellh <username>");
                return;
            }
            ...
            if (User.boissonPrepared == "medkit")
            {
                Name = PlusEnvironment.getNameOfItem("Medkit");
                Price = PlusEnvironment.getPriceOfItem("Medkit");
                Taxe = PlusEnvironment.getTaxeOfItem("Medkit");
            }
            else if (User.boissonPrepared == "heal")
            {
                Name = PlusEnvironment.getNameOfItem("Heal");
                Price = PlusEnvironment.getPriceOfItem("Heal");
                Taxe = PlusEnvironment.getTaxeOfItem("Heal");
            }
            ...
            Session.GetHabbo().addCooldown("sell_hospital", 5000);
            User.boissonPrepared = null;
            User.CarryItem(0);
            User.Say("offers " + TargetClient.GetHabbo().Username + " 1x " + Name + " for $" + Price);
            TargetUser.Transaction = "hospital:" + Name + ":" + Price + ":" + Taxe;
