...
namespace Plus.HabboHotel.Users.UserDataManagement
{
    public class UserDataFactory
    {
        public static UserData GetUserData(string SessionTicket, out byte errorCode)
        {
            ...
            {
                dbClient.SetQuery("SELECT users.id,users.username,users.rank,users.motto,users.look,users.gender,users.last_online,users.ip_last,users.credits,users.activity_points,users.discord_id,users.home_room,users.block_newfriends,users.hide_online,users.hide_inroom,users.vip,users.account_created,users.vip_points,users.machine_id,users.volume,users.chat_preference,users.focus_preference, bubble, name_color, prefix_name, pets_muted,users.bots_muted,users.advertising_report_blocked,users.last_change,users.gotw_points,users.ignore_invites,users.time_muted,users.allow_gifts,users.friend_bar_state,users.disable_forced_effects,users.allow_mimic,users.rank_vip,users.banque,users.timer,users.prison,users.hospital,users.gang,users.gang_rank " +
                "FROM users " +
                "JOIN user_auths " +
                "ON users.id = user_auths.user_id " +
                "WHERE user_auths.auth_ticket = @sso " +
                "LIMIT 1;");
                ...
                dbClient.RunQuery("DELETE FROM `user_auths` WHERE `user_id` = '" + UserId + "' LIMIT 1");
            }
        ...
        public static UserData GetUserData(int UserId)
        {
            ...
            using (IQueryAdapter dbClient = PlusEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                dbClient.SetQuery("SELECT `id`,`username`,`rank`,`motto`,`look`,`gender`,`last_online`,`ip_last`,`credits`,`activity_points`,`home_room`,`block_newfriends`,`hide_online`,`hide_inroom`,`vip`,`account_created`,`vip_points`,`machine_id`,`volume`,`chat_preference`, `focus_preference`,`bubble`, `name_color`,`prefix_name`,`pets_muted`,`bots_muted`,`advertising_report_blocked`,`last_change`,`gotw_points`,`ignore_invites`,`time_muted`,`allow_gifts`,`friend_bar_state`,`disable_forced_effects`,`allow_mimic`,`rank_vip`,`health`,`health_max`,`energy`,`telephone`,`telephone_name`,`telephoneForfaitTel`,`telForfaitSms`,`telForfaitType`,`telForfaitReset`,`banque`,`timer`,`prison`,`hospital`,`arrests`,`kills`,`deaths`,`coca`,`fanta`,`blur`,`audia8`,`porsche911`,`fiatpunto`,`volkswagenjetta`,`bmwi8`,`cb`,`sucette`,`pain`,`doliprane`,`mutuelle`,`mutuelle_expiration`,`gang`,`gang_rank`,`bat`,`sabre`,`ak47`,`ak47_munitions`,`uzi`,`uzi_munitions`,`pierre`,`confirmed`,`coiffure`,`policecasier`,`carte`,`facebook`,`creations`,`permis_arme`,`clipper`,`weed`,`philipmo`,`event_day`,`event_points`,`casino_jetons`,`quizz_points`,`cocktails`,`white_hover`,`audia3`,`eau`,`casier_weed`,`casier_cocktails`,`shifts`,`god_protection`,`passive`,`angelrute`,`tuna`,`catfish`,`salmon`,`hummer`,`carrots`,`shovel`,`lockpick`,`combat_level`,`combat_xp`,`farming_level`,`sales`,`cheetos` FROM `users` WHERE `id` = @id LIMIT 1");
