...
namespace Plus.HabboHotel.Users.Authenticator
{
    public static class HabboFactory
    {
        public static Habbo GenerateHabbo(DataRow Row, DataRow UserInfo)
        {
            ...
            // Makes discord id set to 0 if it does not exist.
            try
            {
                discord_id = Convert.ToInt64(Row["discord_id"]);
            } catch (Exception e)
            {
                discord_id = 0;
            }
            ...
            return new Habbo(Convert.ToInt32(Row["id"]), Convert.ToString(Row["username"]), Convert.ToInt32(Row["rank"]), Convert.ToString(Row["motto"]), Convert.ToString(Row["look"]),
                Convert.ToString(Row["gender"]), Convert.ToInt32(Row["credits"]), Convert.ToInt32(Row["activity_points"]), discord_id,
                ...
                PlusEnvironment.EnumToBool(Row["chat_preference"].ToString()), PlusEnvironment.EnumToBool(Row["focus_preference"].ToString()), Convert.ToInt32(Row["bubble"]), Convert.ToString(Row["name_color"]), Convert.ToString(Row["prefix_name"]), PlusEnvironment.EnumToBool(Row["pets_muted"].ToString()), PlusEnvironment.EnumToBool(Row["bots_muted"].ToString()),
                ...
                PlusEnvironment.EnumToBool(Row["allow_mimic"].ToString()), Convert.ToInt32(Row["rank_vip"]), Convert.ToInt32(Row["banque"]), Convert.ToInt32(Row["timer"]), false, Convert.ToInt32(Row["prison"]), Convert.ToInt32(Row["hospital"]), Convert.ToInt32(Row["gang"]), Convert.ToInt32(Row["gang_rank"]));
        }
    }
}